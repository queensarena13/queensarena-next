...
function toRow(item, target, season) {
  ...
  return {
    ...
    season: String(item.league?.season || season),
...
for (const league of leagues) {
  for (const season of league.seasons) {
    ...
    if (data.errors && Object.keys(data.errors).length > 0) {
      console.warn(
        `${league.name} ${season} unavailable`,
        data.errors
      )
      await sleep(7000)
      continue
    ...
    for (const item of data.response || []) {
      const row = toRow(item, league, season)
    ...
    console.log(`${league.name} ${season}: ${data.response?.length || 0}`)
    await sleep(7000)
  }
}
...
console.log(
  `Synced ${rows.length} historical football matches and ${teamRowsByName.size} teams.`
)
